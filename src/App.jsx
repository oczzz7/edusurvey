        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4 cursor-pointer" onClick={handleSelectionReset}>
            ...
            <div className="hidden sm:flex flex-col">
              <span className="text-slate-900 font-black text-xl tracking-tighter leading-none"><span className="text-indigo-700">EDU</span>SURVEY</span>
          ...
          <div className="flex items-center gap-4">
            {userRole === ROLES.RESPONDENT ? (
              <div className="flex gap-2">
                <button onClick={() => { setUserRole(ROLES.SCHOOL_VIEWER); setCurrentView('stats'); }} className="text-xs font-bold text-slate-500 hover:text-indigo-600 px-3 py-1 flex items-center gap-1"><BarChart2 className="w-4 h-4" /> ანალიტიკა</button>
                <button onClick={() => { setUserRole(ROLES.SUPER_ADMIN); setCurrentView('admin'); }} className="text-xs font-bold text-slate-500 hover:text-indigo-600 px-3 py-1 flex items-center gap-1 border-l pl-3 border-slate-200"><Settings className="w-4 h-4" /> მართვა</button>
              </div>
            ) : (
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-black uppercase bg-indigo-50 border border-indigo-100 text-indigo-700 px-3 py-1.5 rounded-lg shadow-sm">{userRole === ROLES.SUPER_ADMIN ? 'ადმინისტრატორი' : 'ანალიტიკოსი'}</span>
                <button onClick={() => { setUserRole(ROLES.RESPONDENT); handleSelectionReset(); }} className="text-slate-400 hover:text-red-500 transition-colors bg-slate-50 p-2 rounded-lg"><LogOut className="w-5 h-5" /></button>
              </div>
            )}
          </div>
        </div>
            ...
            {currentView === 'landing' && (
              <div className="max-w-2xl mx-auto text-center space-y-12 py-12 animate-in fade-in slide-in-from-bottom-4">
                <div className="bg-white p-12 rounded-[3.5rem] shadow-xl border border-indigo-50/50 space-y-8 relative overflow-hidden">
                  <div className="flex justify-center mb-8"><EmisLogo className="h-20 object-contain drop-shadow-sm" /></div>
...
function GeoSelectionView({ selection, setSelection, onConfirm }) {
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-10 rounded-[3rem] shadow-2xl border border-indigo-50 space-y-6">
      <h2 className="text-3xl font-black text-slate-900 text-center tracking-tighter italic">სკოლის შერჩევა</h2>
      <div className="space-y-4">
        <select className="w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none focus:ring-2 ring-blue-500 transition-all" value={selection.region} onChange={e => setSelection({ region: e.target.value, district: '', schoolId: '' })}><option value="">რეგიონი</option>{geoData.map(r => <option key={r.id} value={r.name}>{r.name}</option>)}</select>
        <select className="w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none disabled:opacity-50" disabled={!selection.region} value={selection.district} onChange={e => setSelection({ ...selection, district: e.target.value, schoolId: '' })}><option value="">რაიონი</option>{reg?.districts?.map(d => <option key={d.name} value={d.name}>{d.name}</option>)}</select>
        <select className="w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none disabled:opacity-50" disabled={!selection.district} value={selection.schoolId} onChange={e => setSelection({ ...selection, schoolId: e.target.value })}><option value="">სკოლა</option>{dist?.schools?.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
      </div>
      <button disabled={!selection.schoolId} onClick={onConfirm} className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black disabled:opacity-30 shadow-xl transition-all active:scale-95">გაგრძელება</button>
    </div>
  );
}
...
function RoleVerificationView({ onVerified, onBack }) {
  ...
  if (role) return (
    <div className="max-w-md mx-auto bg-white p-12 rounded-[3.5rem] shadow-2xl text-center space-y-8 animate-in zoom-in-95">
      <h3 className="text-2xl font-black uppercase text-indigo-600 tracking-widest italic">{roleNames[role]}</h3>
      ...
      <button onClick={() => { setRole(null); setCode(""); }} className="text-slate-400 font-bold hover:text-red-500 transition-colors">უკან დაბრუნება</button>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
      {Object.entries(roleNames).map(([id, label]) => (
        <button key={id} onClick={() => setRole(id)} className="p-10 bg-white border border-slate-100 rounded-[2.5rem] font-black text-xl hover:border-indigo-500 hover:shadow-2xl transition-all flex flex-col items-center gap-4 group">
          <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 group-hover:text-indigo-600 transition-colors uppercase font-black tracking-tighter">{id[0]}</div>
          <span className="text-slate-700 font-black">{label}</span>
        </button>
      ))}
    </div>
  );
...
function GradeSelectView({ onSelect }) {
  return (
    <div className="max-w-md mx-auto space-y-6 text-center">
      <h3 className="text-2xl font-black text-slate-800 mb-6 italic tracking-tight">მიუთითეთ საფეხური</h3>
      {GRADE_LEVELS.map(g => (
        <button key={g} onClick={() => onSelect(g)} className="w-full p-8 bg-white rounded-[2rem] border-2 border-slate-50 font-black text-xl hover:border-indigo-500 hover:bg-indigo-50 transition-all shadow-sm active:scale-95">{g}</button>
      ))}
    </div>
  );
}
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-32 animate-in fade-in slide-in-from-bottom-4">
      <div className="sticky top-20 z-40 bg-white/90 backdrop-blur-md p-6 rounded-3xl border border-slate-200 shadow-lg flex items-center justify-between gap-6">
        <div className="flex-1 space-y-2">
          <div className="flex justify-between text-xs font-black uppercase text-slate-500"><span>პროგრესი: {answeredCount} / {requiredCount}</span><span>{progress}%</span></div>
          <div className="h-3 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-600 transition-all duration-500" style={{ width: `${progress}%` }}></div></div>
        </div>
      </div>
      ...
      <button disabled={answeredCount < requiredCount || isSubmitting} onClick={handleSubmit} className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-sm py-6 bg-blue-600 text-white rounded-[2rem] font-black shadow-2xl disabled:opacity-30 active:scale-95 transition-all">{isSubmitting ? 'იგზავნება...' : 'კითხვარის დასრულება'}</button>
    </div>
  );
}
